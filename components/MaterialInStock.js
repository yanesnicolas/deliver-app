...
const MaterialInStock = ({ navigation }) => {
  const [dataList, setDataList] = useState([]);
  const [filterText, setFilterText] = useState("");
  useEffect(() => {
    const unsubscribe = onSnapshot(
      collection(db, "Material-In-Stock"),
      (snapshot) => {
      ...
      <View View style={{ Width }}>
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Item name={item.name} description={item.email} uri={item.uri} onPress={() => navigation.navigate("Details", { item })} item={item} onDelete={() => deleteItem(item.id)} />
            )
          }}
